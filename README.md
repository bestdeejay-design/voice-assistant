...
ответ по базе знаний проекта Lovii, с цитированием источника. Полностью
локально и бесплатно: без платных API и без облачных сервисов.
...
[Микрофон] → Whisper (STT) → текст
  ...
  → генерация ответа (Ollama deepseek-r1) + цитаты
...
- **LLM (генерация)**: Ollama `deepseek-r1`
